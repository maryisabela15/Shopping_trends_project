...
with tab2:
   ...
   st.subheader('Total female and male customers by region')
   st.write(result)

with tab3:
   st.subheader('Total items purchased by gender in each category')
   query = '''
      ...
      GROUP BY 1, 2;
   '''
   result = pd.read_sql_query(query, engine).pivot_table(values='total_item_purchased', index='category', columns='gender')
   st.write(result)

with tab4:
   st.subheader('Total items purchased by gender in each season')
   query = '''
      ...
      ORDER BY 1,3 DESC;
   '''
   result = pd.read_sql_query(query, engine).pivot_table(values='total_item_purchased', index='season', columns='gender')
   st.write(result)
